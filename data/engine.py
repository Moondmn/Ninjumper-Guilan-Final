...
def convert_matrix(matrix):
    # Unpack the rows of the matrix using the zip function
    # and convert the zip object to a list
...
class physics_obj(object):
    ...
    def move(self, movement, platforms, ramps=[]):
        ...
        platform_conv = convert_matrix(platforms)
        block_hit_list = collision_test(self.rect, platform_conv[0])
        collision_types = {
            "top": False,
            "bottom": False,
            "right": False,
        ...
        for block in block_hit_list:
            ...
            if movement[0] > 0:
                ...
                markers[1] = True
            collision_types["data"].append([block, markers, platforms])
            self.x = self.rect.x
        else:
            collision_types["data"].append(
                ["air", [False, False, False, False], [0, [0, 0]]]
            )
        self.y += movement[1]
...
class cuboid(object):
    ...
    def collidecuboid(self, cuboid_2):
        ...
        if (cuboid_1_xy.colliderect(cuboid_2_xy)) and (
...
def flip(img, x, y):
    return pygame.transform.flip(img, x, y)
...
class entity(object):
    ...
    def get_current_img(self):
        if self.animation == None:
            if self.image != None:
                return flip(self.image, self.flip_x, self.flip_y)
            else:
                return None
        else:
            return flip(
                animation_database[self.animation[self.animation_frame]],
                self.flip_x,
                self.flip_y,
            )
    ...
    def get_drawn_img(self):
        ...
        if self.animation == None:
            if self.image != None:
                image_to_render = flip(self.image, self.flip_x, self.flip_y).copy()
        else:
...
class particle(object):
    def __init__(
        self, x, y, particle_type, motion, decay_rate, start_frame, custom_color=None
    ):
        self.x = x
        self.y = y
